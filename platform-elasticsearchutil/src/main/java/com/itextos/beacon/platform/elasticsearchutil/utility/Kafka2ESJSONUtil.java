...
import com.itextos.beacon.inmemory.elasticsearchcolumn.ESIndexColMapValue;
import com.itextos.beacon.inmemory.elasticsearchcolumn.ElasticSearchColumnDetail;
...
public class Kafka2ESJSONUtil
{
            ...
            IMessage iMsg)
    {
        ...
        subJSON.put(Kafka2ESConstants.ESIndexUniqueColumn, msgId);

        final String dataUpdTime = DateTimeUtility.getFormattedCurrentDateTime(DateTimeFormat.DEFAULT);
        subJSON.put(Kafka2ESConstants.subUpdTmColumn, dataUpdTime);

        final ElasticSearchColumnDetail lAppConfiguration = (ElasticSearchColumnDetail) InmemoryLoaderCollection.getInstance().getInmemoryCollection(InmemoryId.ELASTIC_SEARCH_COLUMN);

        for (final ESIndexColMapValue esColMap : lAppConfiguration.getElasticSearchColumnDetail("submission"))
        {
            ...
            String baseMsgId)
    {
        ...
        fmsgJSON.put(Kafka2ESConstants.ESFmsgIndexUniqueColumn, baseMsgId);
            ...
            IMessage iMsg)
    {
        ...
        delJSON.put(Kafka2ESConstants.ESIndexUniqueColumn, msgId);

        final String dataUpdTime = DateTimeUtility.getFormattedCurrentDateTime(DateTimeFormat.DEFAULT);
        delJSON.put(Kafka2ESConstants.delUpdTmColumn, dataUpdTime);

        final ElasticSearchColumnDetail lAppConfiguration = (ElasticSearchColumnDetail) InmemoryLoaderCollection.getInstance().getInmemoryCollection(InmemoryId.ELASTIC_SEARCH_COLUMN);

        for (final ESIndexColMapValue esColMap : lAppConfiguration.getElasticSearchColumnDetail("delivery"))
        {
            final String  colName       = esColMap.ColumnName;
            final String  colType       = esColMap.ColumnType;
            final String  mapColName    = esColMap.MappedName;
