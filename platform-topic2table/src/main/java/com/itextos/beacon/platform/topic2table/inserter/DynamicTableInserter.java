...
public class DynamicTableInserter
        extends
        AbstractTableInserter
{

    private static final Log                          log                     = LogFactory.getLog(DynamicTableInserter.class);
    private final Map<String, ConnectionAndStatement> mConnectionStatementMap = new HashMap<>();

    public DynamicTableInserter(
    ...
    public ConnectionAndStatement getConnectionAndStatement(
            ...
            throws Exception
    {
        final Map<String, String> map = T2TUtility.getKeyValueMapFromMessage(mTableInserterInfo.getTableNameFinderKeys(), aCurrentMessage);

        mTableNameFinder.setInputValues(mTableInserterInfo.getJndiInfo(), mTableInserterInfo.getDatabaseName(), mTableInserterInfo.getTableName(), map);
        ...
        final String   tempQuery     = T2TUtility.replaceTableName(mInsertQuery, curTableName);
