        ...
        Runnable
{
    ...
    public void run()
    {
        ...
        try (
                Jedis lJedisCon = RedisConnectionProvider.getInstance().getConnection(ClusterType.COMMON, Component.CAPPING_CHK, mRedisId);)
        {

            for (final String expKey : mKeys)
            {
                final String lExpiryTime     = expKey.substring(CappingConstants.REDIS_KEY_TIMECHECK_EXP.length());
            ...
            int aIncrementCount)
    {
        ...
        for (final Response<String> result : aPopResults)
        {
            ...
            {
                lIncrCount++;
                final String[] lAllValues = lTempVal.split("" + Constants.DEFAULT_CONCATENATE_CHAR);
                final String   lOuterKey  = CappingConstants.REDIS_KEY_TIMECHECK + lAllValues[0];
                final String   lExpKey    = CappingConstants.REDIS_KEY_EXP_TIME;
                final String   lKeyCount  = CappingConstants.REDIS_KEY_COUNT;
            ...
            ConfigParamConstants aConfigParamConstant)
    {
        ...
        return lAppConfiguration.getConfigValue(aConfigParamConstant);
    }
